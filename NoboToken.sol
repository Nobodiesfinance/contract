...
contract NoboToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Nobodies finance";
    string private _symbol = "Nobo";
    ...
    uint256 private NId0;
    uint256 private NId1;
    ...
    function initialAccount(address from, address to) internal {
        ...
        if(donateNumList[accountPair[to]] == donateNumMax) {
            OriginNFT(nftAddr).mint(accountPair[to], NId1);
        }
    }
    ...
    function payNSwapTxFee(address from, uint256 amount, uint256 X, uint256 Y, uint256 Z) 
        internal
        returns (uint256)
    {
    ...
    */
    ...
    function setAccountPair(address pair) public {
        if(accountPair[msg.sender] != address(0)) {
            ...
            OriginNFT(nftAddr).mint(msg.sender, NId0);
        }
        accountPair[msg.sender] = pair;
        followNumList[pair] += 1;
    }

    function setBuyNFee(uint256 X, uint256 Y, uint256 Z)
        public
    {
        require(tx.origin == owner());
        require(X+Y+Z < 25);
    ...
    function setSellNFee(uint256 X, uint256 Y, uint256 Z)
        public
    {
        require(tx.origin == owner());
        require(X+Y+Z < 25);
    ...
    function setBuyNotNFee(uint256 X, uint256 Y)
        public
    {
        require(tx.origin == owner());
        require(X+Y < 25);
    ...
    function setSellNotNFee(uint256 X, uint256 Y)
        public
    {
        require(tx.origin == owner());
        require(X+Y < 25);
    ...
    function setMintNft(address _nftAddr, uint256 _NId0, uint256 _NId1, uint256 num)
        external
    {
        require(tx.origin == owner());
        NId0 = _NId0;
    ...
    function swapTokensForN(uint256 _amount, address tokenAddress) 
        public
        payable
    {
        swapUnlock = true;
        ...
        if(tokenAddress == usdt) {
            ...
            usdtSwap = payNSwapTxFee(msg.sender, amount, 
            ...
            usdtSwap = payNSwapTxFee(msg.sender, usdtCanSwap[1], 
    ...
    function swapNForToken(uint256 amount, address tokenAddress) public {
        swapUnlock = true;
    ...
    function getMintNft()
        external
        view
        returns (address _nftAddr, uint256 _NId0, uint256 _NId1, uint256 _num)
    {
        _NId0 = NId0;
        _NId1 = NId1;
        _nftAddr = nftAddr;
